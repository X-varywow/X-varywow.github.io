...
```bash
hook DirectX/OpenGL/Vulkan

# eg. 如下事件触发时截获 GPU 纹理，几乎零额外 COPY
...
目前方案：
- python mss （比 ImageGrab.grab 快很多）
...
rgb 3个通道耦合，仅亮度变化就容易引起3个变量大幅变化

HSV(hue色相，saturation饱和度，value亮度)，天然对红色血条（随亮度变化的）更容易写规则
